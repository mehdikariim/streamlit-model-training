...
def main():
    st.set_page_config(
        page_title="Automated Linear Model Builder",
        layout="wide",
    ...
    with st.container():
        ...
        st.write(
            """
            Welcome to the **Automated Linear Model Builder**! This application empowers you to:
            - **Upload** your dataset (CSV or XLSX).
            - **Define** the target variable.
            - **Adjust** various thresholds (Correlation, VIF, Adjusted R², etc.).
            - **Select** the minimum and maximum number of features.
            - **Train** multiple models and **select** the best-performing ones.
            - **Download** the results, including models and train/test splits, in a single ZIP file.
            """
        )
        st.image("https://images.unsplash.com/photo-1581093588401-6f76f86f8696?ixlib=rb-4.0.3&auto=format&fit=crop&w=1350&q=80", use_column_width=True, caption="Data Science Workflow")
    
    st.markdown("---")
    
    # Sidebar for Parameters and Thresholds
    st.sidebar.header("⚙️ Parameters & Thresholds")
    with st.sidebar.form(key='parameters_form'):
        ...
        submit_parameters = st.form_submit_button(label='Apply Parameters')
    
    st.markdown("---")
    
    # Main Content Area
    st.subheader("📤 Upload Your Dataset")
    uploaded_file = st.file_uploader("Choose a CSV or XLSX file", type=["csv", "xlsx"], help="Upload your dataset in CSV or Excel format.")
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            target_column = st.selectbox("🎯 Select Target Column", all_columns, index=len(all_columns)-1, help="Choose the column you want to predict.")

            # Let user specify which columns to exclude
            st.write("### 🚫 Exclude Unnecessary Columns")
            exclude_columns = st.multiselect("Select Columns to Exclude", options=all_columns, default=[], help="Select columns that should be excluded from the analysis (e.g., ID, irrelevant features).")
            ...
            if st.button("🚀 Train Models"):
                with st.spinner("🔄 Processing... This may take a while depending on data size."):
                    try:
                        ...
                        st.success(f"✅ Reduced features from {X.shape[1]} to {X_reduced.shape[1]} based on correlation threshold.")

                        # 3) Further feature selection via VIF
                        X_selected = select_features_via_vif(X_reduced, threshold=vif_threshold)
                        st.success(f"✅ Selected {X_selected.shape[1]} features after VIF filtering.")
                        ...
                        if not best_models:
                            st.warning("⚠️ No models were trained. Please check your data and parameters.")
                        else:
                            st.balloons()
                            st.success(f"🎉 Found {len(best_models)} models.")

                            # Display interactive tabs for each model
                            st.write("### 🏅 Best Models:")
                            tabs = st.tabs([f"Model {i+1}: {model['Model']}" for i, model in enumerate(best_models)])

                            for i, model in enumerate(best_models):
                                with tabs[i]:
                                    st.write(f"**Model:** {model['Model']}")
                                    st.write(f"**Features:** {', '.join(model['Features'])}")
                                    st.write(f"**R² (Train):** {model['R2_train']:.4f}")
                                    st.write(f"**R² (Test):** {model['R2_test']:.4f}")
                                    st.write(f"**Adjusted R² (Test):** {model['Adjusted_R2']:.4f}")
                                    st.write(f"**MSE:** {model['MSE']:.4f}")
                                    st.write(f"**RMSE:** {model['RMSE']:.4f}")
                                    st.write(f"**MAE:** {model['MAE']:.4f}")
                                    st.write(f"**CV R²:** {model['CV_R2']:.4f}")
                                    st.write("**Equation:**")
                                    st.code(model['Equation'], language='python')

                                    # Plot VIF
                                    st.write("**VIF (Variance Inflation Factor):**")
                                    fig_vif, ax_vif = plt.subplots(figsize=(8, 4))
                                    sns.barplot(x='VIF', y='Feature', data=model['VIF'], ax=ax_vif, palette="viridis")
                                    ax_vif.set_title("VIF per Feature")
                                    ax_vif.set_xlabel("VIF")
                                    ax_vif.set_ylabel("Feature")
                                    st.pyplot(fig_vif)

                                    # Plot Actual vs Predicted
                                    st.write("**Actual vs Predicted (Test Set):**")
                                    y_test = model['y_test']
                                    y_pred = model['Best Estimator'].predict(model['X_test'])
        ...
        st.info("ℹ️ Please upload a CSV or XLSX file to begin.")
