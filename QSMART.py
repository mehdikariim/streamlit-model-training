...
def main():
    ...
    st.markdown(
        """
        **Data to models, faster than ever before.**  
        Save time and focus on results — just upload your data and let the tool handle the rest.
        """
    )
    st.markdown("---")
    ...
    uploaded_file = st.file_uploader(
        "Choose a CSV or XLSX file", type=["csv", "xlsx"], help="Upload your dataset in CSV or Excel format."
    )
                ...
                with st.spinner("🔄 Processing... This may take a while depending on data size."):
                    try:
                        ...
                        if not best_models:
                            ...
                            st.write(f"**Features:** {', '.join(top_model['Features'])}")
                            ...
                            st.write("**Equation:**")
                            st.code(top_model['Equation'], language='python')
